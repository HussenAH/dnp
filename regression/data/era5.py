...
import regression.utils.helper as helper_utils
...
era5_dataset_path = os.path.join(datasets_path, 'ERA5', 'raw')


class ERA5Dataset(utils.Dataset):

    # TODO: validate min and max context points
    def __init__(self, mode: str = 'train', Min_n_cont=2, Max_n_cont=200, circular=True, normalize=True,
                 place='US'):  # mode: train, valid, test
        '''
        ...
        path_to_nc_file = f"{era5_dataset_path}/{place}/netcdf/{mode}_ERA5_{place}.nc"
        ...
        self.X_tensor = torch.stack(
            [self.Longitude.repeat_interleave(self.n_per_axis), self.Latitude.repeat(self.n_per_axis)],
            dim=1)  # .view(self.n_per_axis,self.n_per_axis,2)
        ...
        if self.circular:
            self.circular_indices = helper_utils.get_inner_circle_indices(self.n_per_axis, flat=True)

        # Control inputs:
        if not isinstance(self.Min_n_cont, int) or not isinstance(self.Max_n_cont,
                                                                  int) or self.Min_n_cont > self.Max_n_cont \
                or self.Min_n_cont < 2:
            print(
                "Error: Combination of minimum and maximum number of context points and number of total points not compatible.")
    ...
    def __getitem__(self, idx):
        ...
        """
        Return a single observation of the dataset for the given index.
        """
        # Get the map data (longitude, latitude) and variables (weather data)
        X, Y = self.get_map(idx)

        # Optionally, normalize or perform transformations (if required)
        if self.normalize:
    ...
    def print_report(self):
        ...
        print("=" * 30)
        print(f"Data Shape          : {self.Y_data.shape}")
        print(f"Variables           : {', '.join(self.variables)}")
        print(f"Longitude Range     : {self.Longitude[0].item()}° to {self.Longitude[-1].item()}° (West-East)")
        print(f"Latitude Range      : {self.Latitude[0].item()}° to {self.Latitude[-1].item()}° (South-North)")
        print(f"Grid Points per Map : {self.n_points_per_obs}")
        print("=" * 30 + "\n")
    ...
    def compute_normalization(self):
        ...
        Y_mean = torch.tensor(self.Y_data.mean(dim=['datetime', 'Longitude', 'Latitude']).values,
                              dtype=torch.get_default_dtype())

        # Compute the standard deviation for the data (not correct for wind components!):
        Y_std = torch.tensor(self.Y_data.std(dim=['datetime', 'Longitude', 'Latitude']).values,
                             dtype=torch.get_default_dtype())

        # Correct normalizing for the wind components:
        Y_mean[self.ind_wind_10] = torch.tensor([0., 0.], dtype=torch.get_default_dtype())
        mean_norm_10m = np.linalg.norm(self.Y_data.loc[:, :, :, ['wind_10m_east', 'wind_10m_north']].values,
                                       axis=3).mean()
    ...
    def get_batch(self, inds, transform=False, n_context_points=None, cont_in_target=False):
        ...
        if cont_in_target:
            ...
            return (
                X[:, :n_context_points], Y[:, :n_context_points], X[:, n_context_points:],
                Y[:, n_context_points:, [2, 3]])
    ...
    def get_rand_batch(self, batch_size, transform=False, n_context_points=None, cont_in_target=False):
        ...
        return (self.get_batch(inds=inds, transform=transform, n_context_points=n_context_points,
                               cont_in_target=cont_in_target))
...
def era5_to_task(batch, num_ctx=None, cont_in_target=False, min_ctx_points=2, max_ctx_points=1200,device=None ):
    ...
    if num_ctx is None:
        num_ctx = np.random.randint(low=min_ctx_points, high=max_ctx_points)

    ret_batch.x = X.to(device)
    ret_batch.y = Y.to(device)
    ret_batch.xc = X[:, :num_ctx].to(device)
    ret_batch.yc = Y[:, :num_ctx].to(device)
    if cont_in_target:
        ret_batch.xt = X.to(device)
        ret_batch.yt = Y.to(device)
    else:
        ret_batch.xt = X[:, num_ctx:].to(device)
        ret_batch.yt = Y[:, num_ctx:].to(device)
    #
...
def task_to_era5(batch):
    context = Dict()
    context.x = batch.xc
    context.y = batch.yc
    concat = Dict()
    concat.x = torch.cat([batch.xc, batch.xt], dim=1)
    concat.y = torch.cat([batch.yc, batch.yt], dim=1)
